...
export default class AmazonReloadBalance {
  ...
  async reload(amount, cardNumber) {
    const areInputsValid = AmazonReloadBalance.validateReloadInputs(amount, cardNumber)
    ...
    try {
      ...
      const last4DigitIndex = last4DigitCardNumbers.indexOf(last4Digits) + elementOffset
      ...
      const reloadButtonText = await this.nightmare
        ...
        .evaluate(amountInputSelector => document.querySelector(amountInputSelector).value = '', amountInputSelector)
      ...
      await this.nightmare
        .click(reloadButtonSelector)
        .wait('.a-alert-success')
    } catch (e) {
      console.error(e)
      return false
    }
  ...
  async getLast4DigitCardNumbers() {
    try {
      const last4DigitCardNumbers = await this.nightmare
        ...
        .evaluate(() => {
          const last4DigitCardNumberElements = document.querySelectorAll('.pmts-instrument-acct-number-tail')
          const last4DigitCardNumberList = Array.prototype.slice.call(last4DigitCardNumberElements)
